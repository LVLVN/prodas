...
class Crack500Dataset(Dataset):
    def __init__(self, data_dir, split='train', transform=None, debug=False):
        ...
        if split == 'train':
            self.img_dir = self.data_dir / 'traincrop'
        else:
            self.img_dir = self.data_dir / 'testcrop'
        
        self.image_files = [f for f in os.listdir(self.img_dir) if f.endswith('.jpg')]
        logger.info(f"Found {len(self.image_files)} images in {self.img_dir}")
    ...
    def _validate_image_mask_pairs(self):
        ...
        for img_file in self.image_files[:min(20, len(self.image_files))]:
            ...
            mask_path = self.img_dir / mask_file
    ...
    def __getitem__(self, idx):
        ...
        mask_path = self.img_dir / mask_filename
        ...
        if self.debug and idx < 5:
            ...
            logger.info(f"Resized mask size: {mask.size}")
        ...
        if self.debug and idx < 5:
            ...
            logger.info(f"Image tensor range: [{image.min().item():.4f}, {image.max().item():.4f}]")
            logger.info(f"Mask tensor range: [{mask.min().item():.4f}, {mask.max().item():.4f}]")
